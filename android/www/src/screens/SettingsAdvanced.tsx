...
export function SettingsAdvanced() {
  ...
  const [prootResult, setProotResult] = useState<{
    success: boolean;
    error?: string;
  } | null>(null);
  ...
  function handleOnlineInstall() {
    ...
    const sessionRaw = bridge.callJson<{ id: string; name?: string }>(
      "createSession",
    );
  ...
  function handleRunInTerminal(cmd: string) {
    bridge.call("showTerminal");
    const sessions = bridge.callJson<Array<{ id: string; active: boolean }>>(
      "getTerminalSessions",
    );
  ...
  // Listen for setup progress events to track proot installation status
  useNativeEvent(
    "setup_progress",
    useCallback((data: unknown) => {
      const progress = data as { progress?: number; message?: string };
      if (progress?.progress !== undefined) {
        if (progress.progress >= 1) {
          ...
          // Installation failed (negative progress indicates error)
          setProotResult({
            success: false,
            error: progress.message || "Error en la instalación",
          });
          setProotInstalling(false);
        }
      }
  ...
  function handleInstallProot() {
    ...
    bridge.call("showTerminal");
    // Progress will be tracked via useNativeEvent listener above
  }
    ...
    <div className="page">
        ...
        <>
          ...
          <div className="card">
            ...
            <InfoRow
              label="OpenClaw"
              value={
                versions.openclaw.version !== "not installed"
            ...
            <InfoRow
              ...
              value={
                versions.glibc.ok
                  ? `${versions.glibc.linkerSizeKb} KB`
                  : t("env_not_detected")
              }
          ...
          <div className="card">
            ...
            <InfoRow
              label="glibc linker"
              value={versions.glibc.linkerPath}
              mono
            />
          </div>
        </>
            ...
            {permResult.success ? (
              <>
                ...
                {permResult.errors.length > 0 && (
                  ...
                  >
                    {permResult.errors.length} errores:{" "}
                    {permResult.errors.slice(0, 3).join(", ")}
                  </div>
                )}
              </>
            ) : (
              <div style={{ color: "var(--error)" }}>
                ✗ {permResult.error || "Error desconocido"}
              </div>
            )}
          </div>
        )}
      ...
      <div className="card">
        ...
        {prootResult && (
          <div
            style={{
              marginTop: 10,
              background: prootResult.success
                ? "var(--success-dim)"
                : "var(--error-dim)",
      ...
      <div className="card" style={{ padding: 0, overflow: "hidden" }}>
        {[
          ...
          {
            label: "openclaw gateway --host 0.0.0.0",
            cmd: "openclaw gateway --host 0.0.0.0",
            icon: "▶",
          },
          {
            label: "ls ~/.openclaw-android/bin/",
            cmd: "ls -la ~/.openclaw-android/bin/",
            icon: "📂",
          },
          ...
          <div
            ...
            onKeyDown={(e) =>
              e.key === "Enter" && handleRunInTerminal(item.cmd)
            }
            style={{
              display: "flex",
              alignItems: "center",
